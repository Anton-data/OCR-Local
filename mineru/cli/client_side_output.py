...
def regenerate_derived_outputs(
    parse_dir: str | Path,
    doc_stem: str,
) -> tuple[Path, ...]:
    """Пересобирает .md / content_list.json / content_list_v2.json из УЖЕ
    финализированного middle.json (после ручной правки в визуальном
    редакторе или во вкладке "Список содержимого JSON").

    В отличие от ``regenerate_client_side_outputs``, здесь НЕ вызывается
    ``finalize_client_side_middle_json``: та функция выполняет одноразовую
    финализацию staged middle.json и не идемпотентна -- повторный вызов на
    уже финализированном middle.json может испортить данные. К моменту
    вызова этой функции middle.json уже финализирован (и, возможно, вручную
    отредактирован), поэтому она только читает его как есть и
    детерминированно пересобирает производные форматы через ``union_make``.
    """
    parse_dir = Path(parse_dir)
    middle_json_path = parse_dir / f"{doc_stem}_middle.json"
    markdown_path = parse_dir / f"{doc_stem}.md"
